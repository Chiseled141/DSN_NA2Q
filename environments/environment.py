...
- Supports configurable scenarios via scenario_config.py
"""
...
class DSNEnv(gym.Env):
    ...
    def __init__(
        ...
        target_speed_range: Optional[Tuple[float, float]] = None,
        render_mode: Optional[str] = None,
        seed: Optional[int] = None,
    ):
        super().__init__()
        
        # Load scenario config (allows overrides via parameters)
        cfg = get_scenario_config(scenario)
        
        self.scenario = scenario
        self.grid_size = grid_size or cfg["grid_size"]
        self.n_sensors = n_sensors or cfg["n_sensors"]
        self.n_targets = n_targets or cfg["n_targets"]
        self.cell_size = cell_size or cfg["cell_size"]
        self.sensing_range = sensing_range or cfg["sensing_range"]
        self.fov_angle = np.radians(fov_angle or cfg["fov_angle"])
        self.rotation_step = np.radians(rotation_step or cfg["rotation_step"])
        self.max_steps = max_steps or cfg["max_steps"]
        self.target_speed_range = target_speed_range or cfg["target_speed_range"]
    ...
    def render(self):
        ...
        # Add X and Y axes with coordinates at grid boundaries
