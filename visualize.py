...
def plot_test_results(results: dict, output_path: str, scenario: int = 1):
    ...
    try:
        ...
        history_path = os.path.join(
            script_dir, f"Scenario {scenario} Result", "checkpoints", "training_history.npz"
        )
    ...
    ax1 = axes[0]
    ax1.hist(
        coverage_pct, bins=min(20, n_test_episodes), color="#28A745", alpha=0.7, edgecolor="black"
    )
    ax1.axvline(
...
def generate_video(
    ...
    print(
        f"Scenario {scenario}: {env.grid_size}×{env.grid_size} grid, {env.n_sensors} sensors, {env.n_targets} targets"
    )
