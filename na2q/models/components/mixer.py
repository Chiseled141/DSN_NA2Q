...
class NA2QMixer(nn.Module):
    ...
    def __init__(
        ...
        self.order1_shapes = nn.ModuleList([
            ShapeFunction(input_dim=1) 
            for _ in range(self.n_order1)
        ])
        ...
        self.order2_shapes = nn.ModuleList([
            ShapeFunction(input_dim=2) 
            for _ in range(self.n_order2)
        ])
        ...
        self.semantic_encoder = nn.Sequential(
            nn.Linear(latent_dim * n_agents, attention_hidden_dim),
            nn.ReLU()
        )
